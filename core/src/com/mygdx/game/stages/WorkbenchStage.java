...
public class WorkbenchStage extends OwnStage {
	
	public WorkbenchStage(AON_E game, Inventory inventory) {
		name = "workbench";
		stage = new Stage(game.viewport);
		Table recipeTable = new Table();
		
		ArrayList<CRecipes> workbenchRecipes = new ArrayList<>();
		for (CRecipes recipe: CRecipes.values()) {
			if (recipe.station() == Station.WORKBENCH) {
				workbenchRecipes.add(recipe);
			}
		}
		for (CRecipes recipe: workbenchRecipes) {
		...
		ScrollPane craftingPane = new ScrollPane(recipeTable, AON_E.SKIN);
		
		Table table = new Table();
		table.align(Align.center | Align.top);
		table.setWidth(stage.getWidth());
		table.setHeight(stage.getHeight());
		table.add(craftingPane).padTop(50);
