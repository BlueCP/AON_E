...
public class PolishingStoneStage extends OwnStage {

	public PolishingStoneStage(AON_E game, Inventory inventory) {
		name = "polishing stone";
		stage = new Stage(game.viewport);
		Table recipeTable = new Table();
		
		ArrayList<CRecipes> polishingStoneRecipes = new ArrayList<>();
		for (CRecipes recipe: CRecipes.values()) {
			if (recipe.station() == Station.POLISHING_STONE) {
				polishingStoneRecipes.add(recipe);
			}
		}
		for (CRecipes recipe: polishingStoneRecipes) {
