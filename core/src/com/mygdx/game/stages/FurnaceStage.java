...
public class FurnaceStage extends OwnStage {
	
	public FurnaceStage(AON_E game, Inventory inventory) {
		name = "furnace";
		stage = new Stage(game.viewport);
		Table recipeTable = new Table();
		
		ArrayList<CRecipes> furnaceRecipes = new ArrayList<>();
		for (CRecipes recipe: CRecipes.values()) {
			if (recipe.station() == Station.FURNACE) {
				furnaceRecipes.add(recipe);
			}
		}
		for (CRecipes recipe: furnaceRecipes) {
		...
		ScrollPane furnacingPane = new ScrollPane(recipeTable, AON_E.SKIN);
		
		Table table = new Table();
		table.align(Align.center | Align.top);
		table.setWidth(stage.getWidth());
		table.setHeight(stage.getHeight());
		table.add(furnacingPane).padTop(50);
