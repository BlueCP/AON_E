...
public class SelfcraftingStage extends OwnStage {
	
	public SelfcraftingStage(AON_E game, Inventory inventory) {
		name = "selfcrafting";
		stage = new Stage(game.viewport);
		Table recipeTable = new Table();
		
		ArrayList<CRecipes> selfRecipes = new ArrayList<>();
		for (CRecipes recipe: CRecipes.values()) {
			if (recipe.station() == Station.NONE) {
				selfRecipes.add(recipe);
			}
		}
		for (CRecipes recipe: selfRecipes) {
		...
		ScrollPane selfcraftingPane = new ScrollPane(recipeTable, AON_E.SKIN);
		
		Table table = new Table();
		table.align(Align.center | Align.top);
		table.setWidth(stage.getWidth());
		table.setHeight(stage.getHeight());
		table.add(selfcraftingPane).padTop(50);
